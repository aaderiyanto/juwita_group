...
if($this->cek_login()== TRUE){
		...
		foreach ($list as $person) {
			...
			if($person->flag_po==1 && $person->flag_pembelian==0){
				$sudah_input='<span class="label label-warning">Stok belum dikirim</span>';
			} else 
			if($person->flag_po==1 && $person->flag_pembelian==1){
				$sudah_input='<span class="label label-success">Stok sudah dikirim</span>';
			} else { $sudah_input='<span class="label label-warning">Stok belum dikirim</span>'; }
			$row = array();
			//$row[] = $no.".";
			$row[] = $person->id_po;
