    ...
    <a href="#" class="btn btn-warning" onClick="reload_table()"><i class="icon-loop3 text-white-400"></i></a>
                                ...
                                <table id="table_bmt" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    <thead>
                                        <tr>
                                            <th width="5%">No</th>
                                            <th width="14%">Jenis Asset</th>
                                            <th width="12%">Tanggal</th>
                                            <th>Nama Asset</th>
                                            <th width="12%">Kondisi</th>
                                            <th width="14%">Status Data</th>
...
function add_person()
{
	...
	$('#flag_aktif').val("1").trigger('change');
...
function edit_person(id)
{
    ...
    $.ajax({
        ...
        {
            $('[name="id"]').val(data.id_infoasset);
            $('[name="tanggal"]').val(data.tanggal);
			$('#id_asset').val(data.id_asset).trigger('change');
			$('#kondisi').val(data.kondisi).trigger('change');
                ...
                <form action="#" id="form" class="form-horizontal" autocomplete="off">
                    ...
                    <input type="hidden" value="" name="id"/>
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="tanggal" id="date-modal" value="<?= $config["tgl"]; ?>" placeholder="Text input..." class="form-control" type="text" required>
                                <span class="help-block"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-3">Asset <span class="text-danger">*</span></label>
                            <div class="col-md-9">
                                <select name="id_asset" id="id_asset" class="select-size-lg" type="text" required>
                                <option value="">Pilih :</option>
                                <?php
								$list_sql = $this->db_array("bmt_asset",$field="",$where="1 ORDER BY nama_asset");
								foreach($list_sql as $raw):
								?>
                                <option value="<?= $raw->id_asset; ?>"><?= $raw->nama_asset; ?></option>
								<?php
								endforeach;
								?>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
								<select name="kondisi" id="kondisi" class="select-size-lg" type="text" required>
                                <option value="">Pilih :</option>
                                <?php
								$arr_kondisi=array("Baik","Rusak","Hilang");
								foreach($arr_kondisi as $value):
								?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
								<textarea class="form-control" name="keterangan" id="keterangan" rows="3" placeholder="masukkan isian data" required></textarea>
                                <span class="help-block"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
								<select name="flag_aktif" id="flag_aktif" class="select-size-lg" type="text" required>
                                <?php
								$arr_status=array("1"=>"Aktif","0"=>"Tidak Aktif");
								foreach($arr_status as $id_data=>$value):
								if($id_data ==1){$pilih='selected';} else {$pilih='';}
								?>
                                <option value="<?= $id_data; ?>" <?= $pilih; ?>><?= $value; ?></option>
								<?php
								endforeach;
								?>
                                </select>
