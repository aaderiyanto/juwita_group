...
if($this->post('simpan',true)){
	...
	//$nomor_po = $this->post("no_invoice");
	$arr_po = $this->data_array("pos_po","LEFT(tgl_po,7)='".substr($this->post("tgl_po"),0,7)."' ORDER BY RIGHT(nomor_po,4) DESC");
	if(!empty($arr_po->nomor_po)){
		$urutan = substr($arr_po->nomor_po,-4);
		$nomor_po='PO/'.substr($this->post("tgl_po"),2,2).substr($this->post("tgl_po"),5,2).'/'.substr('000'.($urutan+1),-4);
	} else { $nomor_po='PO/'.substr($this->post("tgl_po"),2,2).substr($this->post("tgl_po"),5,2).'/0001'; }
	// Save
	$data_array = array("id_po" => $id_primary,
	...
	$sql = $this->db_insert($data_array,"pos_po");
  	if($sql == TRUE) { 
                            ...
                            <!--<div class="info_error"></div>-->
								<fieldset class="content-group">
                                    <div class="form-group">
										...
										<div class="col-lg-10">
										<input type="text" name="nomor_po" id="nomor_po" class="form-control" placeholder="jika kosong, akan di generate otomatis" autofocus="autofocus"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Tanggal PO <span class="text-danger">*</span></label>
										<div class="col-lg-10">
										<input type="text" name="tgl_po" id="date_balqon1" class="form-control" placeholder="text input..." required="required" value="<?= $config['tgl']; ?>"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
										...
										<div class="col-lg-10">
										<input type="number" name="terms" id="terms" class="form-control" placeholder="text input..." required="required" value="7"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Ship To <span class="text-danger">*</span></label>
										<div class="col-lg-10">
											<textarea name="ship_to" id="ship_to" class="form-control" rows="3" placeholder="masukkan isian data" required="required"></textarea>
										</div>
									</div>
            ...
            <div style="padding:5px;"></div>
				...
				<tbody>
					<tr id='addr0'>
						<td>1.</td>
						<td>
                        <input type="hidden" name="urutan1" value="1"/>
						<div class="input-group">
							<input type="text" name='produk[]' id="produk1" placeholder='masukkan isian data' class="form-control produk" required="required" onclick="nokeyup()" style="cursor:not-allowed;"/>
                            <span class="input-group-addon modal-produk" data-toggle="modal" data-target="#modal_informasi" data-id="1" data-backdrop="static" data-keyboard="false" style="cursor:pointer;" title='pilih produk'><i class="icon-list"></i></span>
						</div>
                        <div style="padding:3px;"></div>
                        <span id="produk_info1" class="text-primary"></span>
                        <input type="hidden" name="id_barang[]" id="id_produk1" class="id_barang_awal brg1"/>
                        <input type='hidden' name='stok_info[]' id="stok_info1"/>
						</td>
                        <td>
						<input type="text" name="jumlah[]" class="form-control qty1" id="jumlah_awal" value="0" required placeholder="masukkan isian data" required onblur='subtotal(1),findTotal(1)'>
						</select>

						</td>
						<td style="display:none;">
						<input type="text" name="nominal[]" class="form-control nominal_awal nominal1" value="0" required placeholder="masukkan isian data" id="non_idr1" onkeyup="LoadIDRupiah(1)" onblur="subtotal(1),findTotal(1)">
						</td>
                        <td style="display:none;">
                        <!-- SUB TOTAL -->
						<input type="text" name="subtotal_modal[]" class="form-control subtotal_modal" required placeholder="masukkan isian data" style="text-align:right; cursor:not-allowed;" value="0" readonly="readonly" id="subtotal_modal1">
						</td>
					</tr>
				</tbody>
                </div>
                	<tr style="display:none;">
                        ...
                        <td align="right"><span id="grandtotal">Rp. 0</span>
                        <input type="hidden" name="grandtotal" class="form-control grandtotal-form" id="grandtotal-form" required placeholder="masukkan isian data" style="text-align:right;" value="0">
                        </td>
                    </tr>
                    <tr style="display:none;">
                    	<td colspan="4" align="right">PPN 10% : </td>
                        <td align="right"><span id="ppn_rupiah">Rp. 0</span></td>
                    </tr>
                	<tr style="display:none;">
                    	<td colspan="4" align="right">Grand Total : </td>
                        <td align="right"><span id="grandmaster">Rp. 0</span>
                        <input type="hidden" name="grandmaster" id="grandmaster-form" value="0"/>
                        </td>
                    </tr>
			</table>
            </div>
            <input type="hidden" id="jml_count" value="1">
	  ...
	  $("#add_row").click(function(){
		...
		data   +="<td style='vertical-align:top;'>"+
				...
				"</td>"+
				 ...
				 "<td style='display:none;'><input  name='nominal[]' type='text' placeholder='masukkan isian data' value='0' class='form-control nominal"+(i+1)+"' required='required' id='non_idr"+(i+1)+"' onkeyup='LoadIDRupiah("+(i+1)+")' onblur='subtotal("+(i+1)+"),findTotal("+(i+1)+"),marginNominal("+(i+1)+")'></td>"+
				 "</td>"+
				 "<td style='display:none;'>"+
		...
		function cekstok(id)
		{
			var stok = parseFloat($("#stok_info"+id).val());
			var jumlah_qty = parseFloat($(".qty"+id).val());
			if(stok<jumlah_qty){ 
				alert("Stok tidak mencukupi. Maksimal adalah "+stok);
				$(".qty"+id).val(stok); 
				subtotal(id);
				findTotal(id);
			}
