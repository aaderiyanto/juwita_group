...
if($this->post('simpan',true)){
	$id_primary	= "GPB".$this->kode_uniq(14).rand(100,999);
	$from_replace = [".",","]; $to_replace = ["","."];
	$batas_tempo = date('Y-m-d', strtotime($this->post("tgl_invoice").'+'.$this->post("terms").' days'));
	$arr_beli = $this->data_array("gudang_pembelian","LEFT(tgl_invoice,7)='".substr($this->post("tgl_invoice"),0,7)."' AND auto_inv='1' ORDER BY RIGHT(no_invoice,4) DESC");
	if(!empty($arr_beli->no_invoice)){
		$urutan = substr($arr_beli->no_invoice,-4);
		$no_invoice='PMB/'.substr($this->post("tgl_invoice"),2,2).substr($this->post("tgl_invoice"),5,2).'/'.substr('000'.($urutan+1),-4);
	} else { $no_invoice='PMB/'.substr($this->post("tgl_invoice"),2,2).substr($this->post("tgl_invoice"),5,2).'/0001'; }
	if($this->post("no_invoice")!=''){$no_invoice=$this->post("no_invoice");} else {$no_invoice=$no_invoice;}
	// Save
	$data_array = array("id_gudangpembelian" => $id_primary,
	...
	$sql = $this->db_insert($data_array,"gudang_pembelian");
  	if($sql == TRUE) { 
		...
		foreach($arr_barang as $id_produk){
			...
			$id_primary_sub	= "PMD".$this->kode_uniq(14).rand(100,999);
			// Save
			$data_array = array("id_pembdetail" => $id_primary_sub,
								"id_gudangpembelian" => $id_primary,
								"id_produk" => $id_produk,
								"jumlah" => $jumlah,
								"nominal" => $nominal,
								"tgl_input" => date("Y-m-d H:i:s"),
			...
			$save=$this->db_insert($data_array,"gudang_pembelian_detail");
			if($save==TRUE){
				...
				if($arr_produk->jenis_data=='stok'){
					$this->data_update("pos_produk","stok_gudang=stok_gudang+".$jumlah,"id_produk='".$id_produk."'");
				}
			}
			$no++;
		}
					...
					<div class="panel-body">
                            ...
                            <!--<div class="info_error"></div>-->
								<fieldset class="content-group">
                                    ...
                                    <div class="form-group">
										<label class="control-label col-lg-2">Terms (hari)<span class="text-danger">*</span></label>
										<div class="col-lg-10">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="id_po" data-placeholder="Pilih :" class="select-size-lg required" id="id_po" onchange="LoadPO()">
                                            <option value="">Pilih :</option>
                                        <?php
										$list_sql = $this->db_array("pos_po a, pos_vendor b",$field="a.*, b.nama",$where="1 AND a.bill_to=b.id_vendor AND a.flag_pembelian='0' AND a.flag_gudang='1' AND flag_po='1' ORDER BY a.tgl_po,a.nomor_po");
										foreach($list_sql as $raw):
										?>
            ...
            <div style="padding:5px;"></div>
				...
				<thead>
					<tr class="bg-teal">
						...
						<th class="text-center" width="16%" <?= $display; ?>>
							Unit Price (Rp) <span class="text-danger">*</span>
						</th>
                        <th class="text-center" width="18%" <?= $display; ?>>
