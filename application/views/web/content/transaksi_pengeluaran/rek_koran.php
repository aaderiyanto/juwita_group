...
$id_katfinance = $this->post("id_katfinance");
if(!empty($id_akun)){$where_akun="AND a.id_akun='".$id_akun."'";} else {$where_akun='';}
if(!empty($id_katfinance)){$where_kat="AND a.id_katfinance='".$id_katfinance."'";} else {$where_kat='';}
...
<tr>
    ...
    <th width="17%">NO. TRANSAKSI</th>
    <th width="17%">AKUN</th>
    <th width="17%">KATEGORI</th>
    <th width="15%">PENGELUARAN (K)</th>
    <th>DESKRIPSI</th>
</tr>
<?php
$sql = $this->db_array("finance_transaksi a",$field="a.*",$where="1 AND a.tgl_transaksi BETWEEN '".$from."' AND '".$to."' AND a.flag_aktif='1' AND a.jenis_data='k' $where_akun $where_kat ORDER BY a.tgl_transaksi,a.no_transaksi");
$no=0;
$subdebit=0;
$subkredit=0;
$subsaldo=0;
...
foreach($sql as $raw):
$no++;
$arr_akun = $this->data_array("finance_akun","id_akun='".$raw->id_akun."'");
$arr_kategori = $this->data_array("finance_kategori","id_katfinance='".$raw->id_katfinance."'");
...
<tr>
	...
	<td><?php echo $arr_akun->nama_akun; ?></td>
	<td><?php echo $arr_kategori->kategori; ?></td>
...
?>
<tr>
	<td colspan="7" align="center" style="color:#F00;">Data tidak ditemukan!</td>
</tr>
<?php
}
?>
