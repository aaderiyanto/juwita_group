...
function LoadSaldo()
{
	...
	$.ajax({
		type: 'POST',
		url: '<?= $this->base_url($config['folder_apps']).'ajax/'.$url_system[1]; ?>/ajax_load_akun',
		data: {id_akun:id_akun, id:1},
		success: function(data){
			$('#saldo_akun').val(data);
		}
	});
...
function SelisihSaldo()
{
	var inputnominal_1 = $("#non_idr1").val().replace(/\./g, "");
	var inputnominal = parseFloat(inputnominal_1.replace(/\,/g, "."));
	var saldo = parseFloat($("#saldo_akun").val());
	if(inputnominal > saldo){
		$(".info_error").html('<div class="alert alert-danger alert-styled-left alert-arrow-left alert-bordered">'+
						 '<button type="button" class="close" data-dismiss="alert"><span>&times;</span>'+
						 '<span class="sr-only">Close</span></button>'+
					     '<span class="text-semibold"><b>Oopsss!</b></span> Saldo akun tidak mencukupi. saldo saat ini <b>Rp. '+formatCurrency(saldo)+'</b></div>');
		$("#non_idr1").val("0");
	}
}
</script>
			...
			<div id="modal_form" class="modal fade">
                ...
                <form action="#" id="form" class="form-horizontal" autocomplete="off">
                    ...
                    <input type="hidden" value="" name="id"/>
                    	<input type="hidden" name="jenis_data" id="jenis_data" value="k"/>
                        <input type="hidden" name="saldo_akun" id="saldo_akun" value="0"/>
                        <div class="form-group">
                            <label class="control-label col-md-3">Jenis Akun</label>
                            <div class="col-md-9">
                                <select name="id_akun" id="id_akun" data-placeholder="Pilih :" class="select-size-lg" onchange="LoadSaldo()">
                                        <option value="">Pilih :</option>
                                        <?php
										$list_sql = $this->db_array("finance_akun",$field="",$where="1 ORDER BY jenis_akun");
										foreach($list_sql as $raw){
										?>
                                        	<option value="<?php echo $raw->id_akun; ?>"><?php echo $raw->nama_akun." (Saldo: Rp. ".$this->rupiah($raw->saldo).")"; ?></option>
                                        <?php
										}
										?>
								</select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <select name="id_katfinance" id="id_katfinance" data-placeholder="Pilih :" class="select-size-lg">
                                        <option value="">Pilih :</option>
                                        <?php
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input type="text" name="nominal" id="non_idr1" class="form-control" placeholder="text input..." required="required" onkeyup="LoadIDRupiah(1)" onchange="SelisihSaldo()"/>
                                <span class="help-block"></span>
                            </div>
                        </div>
                        <!--<div class="form-group">
