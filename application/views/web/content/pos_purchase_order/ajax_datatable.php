...
if($this->cek_login()== TRUE){
		$where_default = array("a.flag_gudang='0'");
		$table = "pos_po a";
		$column_order = array(null,"a.tgl_po","a.nomor_po","b.nama","a.flag_po");
		$column_search = array("a.tgl_po","a.nomor_po","b.nama","CASE WHEN a.flag_po='1' THEN 'Sdh Disetujui' WHEN a.flag_po='2' THEN 'Pending' WHEN a.flag_po='0' THEN 'Batal' END");
		...
		foreach ($list as $person) {
			...
			if($person->flag_po == 2 && in_array($this->baca_session("level"),array("admin","pimpinan","administrasi"))){
				$row[] = '<a class="" href="'.$this->base_url($config["folder_apps"]).'pos_purchase_order/validasi/'.$person->id_po.'" title="Validasi Data"><i class="glyphicon glyphicon-check"></i> '.$flag_po.'</a>';
			} else {
				$row[] = $flag_po;
			}
			//add html for action
			if($person->flag_po == 2 && !in_array($this->baca_session("level"),array("gudang"))){
			$row[] = '<a class="" href="'.$this->base_url($config["folder_apps"]).'pos_purchase_order/edit/'.$person->id_po.'" title="Edit"><i class="glyphicon glyphicon-pencil"></i></a> &nbsp;
