...
$arr = $this->data_array("bmt_manufacture","id_manufacture='".$url_system[3]."'");
if(!empty($arr->id_manufacture) && $arr->flag_data!=0){
if($this->post('simpan',true)){
	// Save
	$data_array = array("id_vendor" => $this->post("bill_to"),
						"nama_produk" => $this->post("nama_produk"),
						"jumlah" => $this->post("jumlah_produk"),
						"tgl_buat" => $this->post("tgl_buat"),
						"tgl_selesai" => $this->post("tgl_selesai"),
						"keterangan" => $this->post("keterangan"),
						"flag_data" => $this->post("flag_data"),
						"tgl_update" => date("Y-m-d H:i:s"),
						"user_update" => $this->baca_session('id_admin')
					    );
	$sql = $this->db_update($data_array,"bmt_manufacture","id_manufacture='".$arr->id_manufacture."'");
  	if($sql == TRUE) { 
		// DELETE DETAIL BY PRIMARY
		$this->db_delete(array("bmt_manufacture_detail"),"id_manufacture='".$arr->id_manufacture."'");
		...
		foreach($arr_barang as $id_produk){
			...
			$data_array = array("id_manufacture_detail" => $id_primary_sub,
								"id_manufacture" => $arr->id_manufacture,
								"id_produk" => $id_produk,
								"jumlah" => $jumlah,
                            ...
                            <!--<div class="info_error"></div>-->
								<fieldset class="content-group">
                                    <div class="form-group">
										<label class="control-label col-lg-2">Nomor Manufacture </label>
										<div class="col-lg-10">
										<input type="text" name="nomor_mo" value="<?= $arr->nomor_mo; ?>" id="nomor_po" class="form-control" placeholder="jika kosong, akan di generate otomatis" disabled="disabled"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Nama Produk <span class="text-danger">*</span></label>
										<div class="col-lg-10">
										<input type="text" name="nama_produk" value="<?= $arr->nama_produk; ?>" id="nama_produk" class="form-control" placeholder="text input..." required="required"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Jumlah <span class="text-danger">*</span></label>
										<div class="col-lg-10">
										<input type="number" name="jumlah_produk" value="<?= $arr->jumlah; ?>" class="form-control" placeholder="number input..." required="required"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Tanggal Buat <span class="text-danger">*</span></label>
										<div class="col-lg-10">
										<input type="text" name="tgl_buat" id="date_balqon1" class="form-control" placeholder="text input..." required="required" value="<?= $arr->tgl_buat; ?>"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Estimasi Selesai <span class="text-danger">*</span></label>
										<div class="col-lg-10">
										<input type="text" name="tgl_selesai" id="date_balqon2" class="form-control" placeholder="text input..." required="required" value="<?= $arr->tgl_selesai; ?>"/>
                                        </div>                                       
									</div>
                                    <div class="form-group">
										...
										if($arr->id_vendor==$raw->id_vendor){$pilih='selected';} else {$pilih='';}
										?>
                                        ...
                                        <div class="col-lg-1">
                                        	<button type="button" class="btn btn-success" title="tambah bill to" onclick="AddBillto()"><span class="icon-plus2 text-white"></span></button>
                                        </div>                         
									</div>
                                    <div class="form-group">
										<label class="control-label col-lg-2">Status Data <span class="text-danger">*</span></label>
										<div class="col-lg-10">
                                        <select name="flag_data" data-placeholder="Pilih :" class="select-size-lg required">
                                            <option value="">Pilih :</option>
                                        <?php
										$array_flagpo = array("2"=>"Pending","3"=>"Dalam Proses","1"=>"Selesai","0"=>"Batal");
										foreach($array_flagpo as $id_val=>$val_data){
											if($id_val==$arr->flag_data){$pilih='selected';} else {$pilih='';}
										?>
                                    ...
                                    <div class="form-group">
										...
										<div class="col-lg-10">
											<textarea name="keterangan" id="keterangan" class="form-control" rows="2" placeholder="masukkan isian data" required="required"><?= $arr->keterangan; ?></textarea>
										</div>
									</div>
...
<?php
$no=0; $grand_total=0;
$list_sql = $this->db_array("bmt_manufacture_detail",$field="",$where="1 AND id_manufacture='".$arr->id_manufacture."'");
foreach($list_sql as $raw):
$no++;
