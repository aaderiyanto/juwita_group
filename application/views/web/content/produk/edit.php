...
if($this->post('simpan',true)){
	...
	if(!empty($this->post("foto"))){
		$nama_file_upload = "produk_".$arr->id_produk."_".rand(100,999).".png";
	...
	} else { $nama_file_upload = $arr->foto; }
	$from_replace = [".",","]; $to_replace = ["","."];
	$nominal = str_replace($from_replace,$to_replace,$this->post("nominal"));
	$harga_modal = str_replace($from_replace,$to_replace,$this->post("harga_modal"));
	// Save
	...
	$sql = $this->db_update($data_array,"pos_produk","id_produk='".$arr->id_produk."'");
	if($sql == TRUE){ $this->redirect($folder_grid."/home/2"); } // akhir update data
}
?>
...
?>
                                    <div class="form-group">
										<label class="control-label col-lg-3">Harga Modal <span class="text-danger">*</span></label>
										<div class="col-lg-9">
										<input type="text" name="harga_modal" id="non_idr2" class="form-control" placeholder="text input..." required="required" onkeyup="LoadIDRupiah(2)" value="<?= $this->rupiah($arr->harga_modal,2); ?>"/>
                                        </div>                                       
									</div>
<?php } ?>
                                    <div class="form-group">
										<label class="control-label col-lg-3">Harga Jual <span class="text-danger">*</span></label>
										<div class="col-lg-9">
...
<div class="col-lg-4">
									...
									<div class="imagearea">
										...
										if(empty($arr->foto)){
										?>
                                        <img src="<?php echo $this->base_url($config['folder_apps']); ?>images/no_pictures.png" alt="foto"  id="avatarimage" style="width:250px; padding:6px;"/>
                                        <?php } else { ?>
                                        <img src="<?php echo $this->base_url($config['folder_apps']); ?>fupload/produk/<?= $arr->foto; ?>" alt="foto"  id="avatarimage" style="width:250px;"/>
                                        <?php } ?>
